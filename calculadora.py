...
# --- Título general ---
st.markdown("<h1 style='text-align: center; color: #003366;'>Calculadoras BacT/ALERT Plus</h1>", unsafe_allow_html=True)

# --- Crear pestañas ---
tab_volumen, tab_tiempo = st.tabs(["🧪 Calculadora de Volumen", "⏱ Calculadora de Tiempo de Positividad"])
...
# ================================
# TAB 2: CALCULADORA DE TIEMPO DE POSITIVIDAD
# ================================
with tab_tiempo:
    st.markdown("<h3 style='color: #003366;'>Conversión de días a horas</h3>", unsafe_allow_html=True)
    
    dias = st.number_input("Ingrese el tiempo en días", min_value=0.0, step=0.01)

    if dias:
        horas = dias * 24
        horas_int = int(horas)
        minutos = int((horas - horas_int) * 60)
        st.markdown(f"<h3 style='color: blue;'>Resultado: {horas_int} horas y {minutos} minutos</h3>", unsafe_allow_html=True)
